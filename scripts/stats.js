...
export function renderStatsPage() {
  ...
  // Загружаем платежи из localStorage
  ...
  if (!isAdmin) {
    filteredPayments = payments.filter(x => x.ownerEmail === curUser.email);
  }
  ...
  // Для конвертации берём курсы из localStorage (adminRates2)
  const rates = JSON.parse(localStorage.getItem('adminRates2') || 'null') || { RUB: 1, AED: 22, USD: 88, EUR: 94, CNY: 14, GBP: 122 };
  ...
  function renderCharts(){
    let filteredPays = [...filteredPayments];
    // Фильтрация по дате
